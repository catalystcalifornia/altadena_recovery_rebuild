...
# get curr geoms
geoms <- st_read(con_alt, query="Select * from dashboard.assessor_parcels_universe_2026_08") %>%
...
curr_area <- geoms %>% 
  ...
  left_join(xwalk, by="ain_2026_08") %>%
...
curr_area <- curr_area %>%
  distinct(ain_2026_08,area_name,area_label,geom)

# check for duplicates and gaps
curr_area %>%
  group_by(ain_2026_08) %>%
  mutate(count=n()) %>%
  filter(count > 1) %>%
  nrow() # should be 0

nrow(curr_area) - length(unique(curr_area$ain_2026_08)) # should be 0

# check for NA - should be 0
table(curr_area$area_name,useNA='always')
# Dec QA
# East West <NA> # 04/12/2026 and same 8/6/2026
#   1664 4012    0 
...
# Add parcel area
curr_area$parcel_area <- as.numeric(st_area(curr_area))*10.7639 # area for 3310 is in square meets so convert to square feet

# rearrange columns
curr_area <- curr_area %>%
  select(ain_2026_08,area_name,area_label,parcel_area,geom)
...
source <- "Script: W:/Project/RDA Team/Altadena Recovery and Rebuild/GitHub/AB/altadena_recovery_rebuild/altadena_recovery_rebuild/Data Prep/Monthly Updates/rel_assessor_parcels.R "
...
column_comments <- c('Assessor ID number for current month - use this to match to other relational tables',
                     ...
                     'parcel area calculated using st_area so only based on shapefile boundaries and not setbacks or formal lot dimensions',
                     'geometry')
