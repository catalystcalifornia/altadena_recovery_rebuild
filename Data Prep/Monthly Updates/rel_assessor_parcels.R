...
# get curr geoms
geoms <- st_read(con_alt, query="Select * from dashboard.assessor_parcels_universe_2026_04") %>%
...
curr_area <- geoms %>% 
  ...
  left_join(xwalk, by="ain_2026_04") %>%
...
curr_area <- curr_area %>%
  distinct(ain_2026_04,area_name,area_label,geom)

# check for duplicates and gaps
curr_area %>%
  group_by(ain_2026_04) %>%
  mutate(count=n()) %>%
  filter(count > 1) %>%
  nrow() # should be 0

nrow(curr_area) - length(unique(curr_area$ain_2026_04)) # should be 0
...
# rearrange columns
curr_area <- curr_area %>%
  select(ain_2026_04,area_name,area_label,parcel_area,geom)
