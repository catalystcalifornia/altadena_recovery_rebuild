...
# "5823022022" "5823022023" 
# "5827014035" "5827014036" 
# "5830016058" 
# "5831016035" "5831016036"
# "5839016025" "5839016026" 
# "5841007024" 
# "5842008017" "5842008018" 
# "5843023069" "5843023070"
# "5844011016" 
# "5863003900"
...
# "5823022022" - NEW! - https://portal.assessor.lacounty.gov/parceldetail/5823022022
# "5823022023" - NEW! - https://portal.assessor.lacounty.gov/parceldetail/5823022023
# "5830016058" - NEW! - https://portal.assessor.lacounty.gov/parceldetail/5830016058 <-- matched calfire; looks like it should be 5830016052 - https://portal.assessor.lacounty.gov/parceldetail/5830016052
...
# "5841007024" - no response (same) - https://portal.assessor.lacounty.gov/parceldetail/5841007024 <-- matched calfire
...
# "5844011016" - NEW! - https://portal.assessor.lacounty.gov/parceldetail/5844011016 <-- matched calfire; looks like it should be 5844011003 - https://portal.assessor.lacounty.gov/parceldetail/5844011003

## Shells - not sure we can do anything with these - no associated addresses
# "5827014035" - 0100/shell (same) - https://portal.assessor.lacounty.gov/parceldetail/5827014035 <-- matched calfire - no damage 
# "5827014036" - 0100/shell (same) - https://portal.assessor.lacounty.gov/parceldetail/5827014036 <-- leftover / mapsearch shows same AIN (looks to be part of 5827014035) 
# "5839016025" - 0100/shell (same) - https://portal.assessor.lacounty.gov/parceldetail/5839016025 <-- leftover / mapsearch shows same AIN 
# "5842008017" - 0100/shell (same) - https://portal.assessor.lacounty.gov/parceldetail/5842008017 <-- leftover / mapsearch shows same AIN (looks to be part of 5842008018) 
# "5842008018" - 0100/shell (same) - https://portal.assessor.lacounty.gov/parceldetail/5842008018 <-- matched calfire - destroyed residence
# "5843023069" - 0101/shell (same) - https://portal.assessor.lacounty.gov/parceldetail/5843023069 <- matched calfire
...
matched <-  mismatch_1 %>% filter(ain %in% unique(calfire_matches$ain)) # 9
sort(matched$ain)
# "5827014035" "5830016058" "5831016035" "5831016036" "5841007024" 
# "5842008018" "5843023069" "5843023070" "5844011016"

leftover <- mismatch_1 %>% filter(!(ain %in% unique(calfire_matches$ain))) # 7
sort(leftover$ain)
...
mismatch1_damaged$apn_parcel # 10
...
# "5844011003" / "5844011016" - 0100 - https://portal.assessor.lacounty.gov/parceldetail/5844011003
# "5830016052" / "5830016058" - 0101 - https://portal.assessor.lacounty.gov/parceldetail/5830016052

# above confirms that apn_parcel is likely the better ain, 
# we should check that these are in the csv - they are not
all_csv_results <- dbGetQuery(conn = con, sprintf("SELECT * FROM %s.%s;", schema, csv_table_name))
check <- all_csv_results %>% filter(ain %in% mismatch1_damaged$apn_parcel) # 0
n_distinct(mismatch1_damaged$apn_parcel) # 8

### 2. Get AINs from CSVs (based on situs city_state filter), 
### and review which ones are not in the SHP ain universes
...
# combine
all_csv_results <- bind_rows(csv_1_results, csv_2_results, csv_3_results) #11739
...
length(unique(all_csv_results$ain)) #11739
...
mismatch_2 <- data.frame(ain=setdiff(all_csv_results$ain, shp_ain_universe)) %>% # 24
...
# mismatch 2 includes all the shp ains that matched to damaged structures in calfire data
table(mismatch_2$mismatch1_apn, mismatch_2$use_code) 
# Note: There are 10 (in addition to the 8 from mismatch1) that are residential and non-vacant
table(mismatch_2$use_code)

# missing parcel shapes but have CSV data and are considered residential, non-vacant by Assessor
check <- mismatch_2 %>% 
  filter(mismatch1_apn==FALSE & grepl("^0", use_code) & !grepl("V$", use_code)) # 12

check2 <- check %>% 
  mutate(ain=as.character(ain)) %>% 
  left_join(calfire, by=c("ain"="apn_parcel")) %>%
  select(ain, damage, everything()) # 14 - all are NA or No Damage
...
# 5823022006 - map says AIN is 5823022022, no parcel change history, possibly not impacted by Eaton - https://portal.assessor.lacounty.gov/parceldetail/5823022006
...
# 5844011004 - map says AIN is 5844011016, no parcel change history, Misfortune & Calamity Status is in progress, no building: https://portal.assessor.lacounty.gov/parceldetail/5844011004
...
# way of fire damage. This means the NAs from CalFire don't currently appear to have data quality issues
...
# rename_query <- sprintf("ALTER TABLE %s.%s RENAME TO %s_old;", schema, shp_table_name, shp_table_name)
# dbSendQuery(con, rename_query)
...
csv_1_ain_filter <- batch_filter_csv_data(
  csv_file=csv_1,
  target_list = shp_ain_universe,
  filter_column="AIN", # AIN or ï»¿AIN sometimes has encoding typos
  chunk_size = 10000,
  debug_filter=TRUE,
  exact_match=TRUE)
...
# # rename to _old
# rename_query <- sprintf("ALTER TABLE %s.%s RENAME TO %s_old;", schema, csv_table_name, csv_table_name)
# dbSendQuery(con, rename_query)
