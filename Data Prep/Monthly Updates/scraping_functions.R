...
##### New approach waiting for a specific JSON response - more reliable #####
...
# ##### Old approach using html and DOM checks - more unreliable #####
# # The website is a single page application (SPA) will try a longer wait time
# wait_for_spa_load <- function(url, ain=NA, max_wait = 20) {
...
#     b$Page$loadEventFired()
# 
#     log_message("Waiting for SPA to fully load...")
# 
#     start_time <- Sys.time()
# 
...
#   ')$result$value
#       }, error = function(e) {
#         log_message("Warning: Could not check module loaded status")
#         return(FALSE)
#       })
# 
#       log_message(paste("Waiting for results module... (", round(elapsed_time, 1), "s elapsed)"))
# 
#       if(module_loaded) {
#         log_message("✓ Results module loaded!")
# 
#         # Give Angular a beat to finish rendering the ng-repeat list after the label updates
#         Sys.sleep(1.5)
# 
#         # Extract the claimed total from whichever label is showing
#         claimed_total <- tryCatch({
#           b$Runtime$evaluate('
#       (function() {
#         var multi = document.querySelector(\'label[data-ng-show="vm.totalFoundDisplay > 1"][aria-hidden="false"]\');
#         var one = document.querySelector(\'label[data-ng-show="vm.totalFoundDisplay == 1"][aria-hidden="false"]\');
#         var none = document.querySelector(\'label[data-ng-show="vm.totalFoundDisplay == 0"][aria-hidden="false"]\');
#         if (none) return 0;
#         if (one) return 1;
#         if (multi) return parseInt(multi.textContent.match(/\\d+/)[0], 10);
#         return -1;
#       })()
#     ')$result$value
#         }, error = function(e) -1)
# 
#         actual_count <- tryCatch({
#           b$Runtime$evaluate('document.querySelectorAll("div[name=\\"label-SearchResult\\"]").length')$result$value
#         }, error = function(e) 0)
# 
#         log_message(paste("Claimed total:", claimed_total, "| Actual rendered:", actual_count))
# 
#         # If the label says results exist but none rendered yet, wait a bit longer and recheck once
#         if(claimed_total > 0 && actual_count < claimed_total) {
#           log_message("⚠ Count mismatch - results claimed but not yet rendered, waiting...")
#           Sys.sleep(3)
#           actual_count <- tryCatch({
#             b$Runtime$evaluate('document.querySelectorAll("div[name=\\"label-SearchResult\\"]").length')$result$value
#           }, error = function(e) 0)
...
# 
#         if(claimed_total > 0 && actual_count < claimed_total) {
#           log_message("✗ Mismatch persists - flagging as timeout rather than false success")
#           log_mismatch(ain, claimed_total, actual_count)
#           status <- "timeout"
# 
#           # add diagnostic: save a screenshot so you can see what loaded when request timed out
#           tryCatch({
#             b$screenshot(filename = file.path(custom_chrome_dir, paste0("mismatch_debug_", Sys.getpid(), ".png")))
#           }, error = function(e) log_message("Couldn't capture debug screenshot"))
#         } else {
#           status <- "success"
#         }
# 
...
#     if(status != "success") {
#       log_message("⚠ Timeout: Results module did not load within wait period")
#       status <- "timeout"
# 
#       # add diagnostic: save a screenshot so you can see what loaded when request timed out
#       tryCatch({
...
#   
#   # First attempt
#   result <- wait_for_spa_load(url, ain=ain, max_wait = wait_time)
#   
...
#     result <- wait_for_spa_load(url, ain=ain, max_wait = retry_wait_time)
#     is_retry <- TRUE
#     
#     if(result$status == "success") {
#       log_message("✓ Retry successful!")
#     } else {
#       log_message(paste("✗ Retry also failed with status:", result$status))
#     }
#   }
#   
